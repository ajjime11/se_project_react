...
function App() {
  ...
  const [weather, setWeather] = useState({ temperature: 75 });
    ...
    const fetchWeatherData = async () => {
      try {
        const response = await fetch(
          "https://api.openweathermap.org/data/2.5/weather?q=Tempe&appid=YOUR_API_KEY&units=imperial"
        );
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
        const data = await response.json();
        setWeather({ temperature: Math.round(data.main.temp) });
      } catch (error) {
